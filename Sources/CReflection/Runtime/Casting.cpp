...
extern "C" const void *swift_dynamicCastObjCProtocolConditional(
    const void *object, size_t numProtocols, Protocol *const *protocols);
#endif
...
//        while (classType->isTypeMetadata() &&
//        classType->isArtificialSubclass())
...
//        auto objcWrapper = static_cast<const ObjCClassWrapperMetadata
//        *>(type); const char *className =
//        class_getName(class_const_cast(objcWrapper->Class)); result =
//        className; return;
//    }
//#endif
//
...
// void
// swift::swift_dynamicCastFailure(const void *sourceType, const char
// *sourceName,
...
static bool _unknownClassConformsToObjCProtocol(const OpaqueValue *value,
                                                Protocol *protocol) {
  const void *object = *reinterpret_cast<const void *const *>(value);
  return swift_dynamicCastObjCProtocolConditional(object, 1, &protocol);
}
#endif

bool swift::_conformsToProtocol(const OpaqueValue *value, const Metadata *type,
...
//}
//
// static bool shouldDeallocateSource(bool castSucceeded, DynamicCastFlags
// flags) {
...
// { switch (type->getKind()) { case MetadataKind::Existential: { auto
// existentialType = cast<ExistentialTypeMetadata>(type);
//
// switch (existentialType->getRepresentation()) {
// case ExistentialTypeRepresentation::Class:
//// Nothing to clean up.
...
// HeapObject *_bridgeAnythingToObjectiveC(OpaqueValue *src, const Metadata
// *srcType); #endif
//
...
// if (canConsumeDynamicValue && (flags & DynamicCastFlags::TakeOnSuccess)) {
// auto *value =
// srcDynamicType->allocateBoxForExistentialIn(&destExistential->Buffer);
...
//}
//
// swift_runtime_unreachable("Unhandled ExistentialTypeRepresentation in
// switch.");
//}
//
///******************************************************************************/
...
//                // If the target existential requires witness tables, we can't
//                do this cast.
//                // The result type would not have a single-refcounted-pointer
//                rep. return nullptr;
...
//        case MetadataKind::Class: {
//            auto targetClassType = static_cast<const ClassMetadata
//            *>(targetType); return swift_dynamicCastClass(object,
//            targetClassType);
//        }
//
//        case MetadataKind::ObjCClassWrapper: {
...
//                = static_cast<const ObjCClassWrapperMetadata
//                *>(targetType)->Class;
...
//        case MetadataKind::ForeignClass: {
//#if SWIFT_OBJC_INTEROP
//            auto targetClassType = static_cast<const
//            ForeignClassMetadata*>(targetType); return
//            swift_dynamicCastForeignClass(object, targetClassType);
//#else
//            return nullptr;
//#endif
//        }
...
//        case MetadataKind::Class: {
//            auto targetClassType = static_cast<const ClassMetadata
//            *>(targetType); return swift_dynamicCastClassUnconditional(object,
//            targetClassType, file, line, column);
//        }
//
//        case MetadataKind::ObjCClassWrapper: {
//#if SWIFT_OBJC_INTEROP
...
//                = static_cast<const ObjCClassWrapperMetadata
//                *>(targetType)->Class;
//            return swift_dynamicCastObjCClassUnconditional(object,
//            targetClassType, file, line, column);
//#else
//            swift_dynamicCastFailure(_swift_getClass(object), targetType);
//#endif
...
//            auto targetClassType = static_cast<const
//            ForeignClassMetadata*>(targetType); return
//            swift_dynamicCastForeignClassUnconditional(object,
//            targetClassType, file, line, column);
//#else
//            swift_dynamicCastFailure(_swift_getClass(object), targetType);
//#endif
...
//
//        case MetadataKind::Existential: {
//            // We can cast to ObjC existentials. Non-ObjC existentials don't
//            have
...
//            targetType = static_cast<const
//            ObjCClassWrapperMetadata*>(targetType)
//                ->Class;
//            LLVM_FALLTHROUGH;
//        case MetadataKind::Class:
//            // The source value must also be a class; otherwise the cast
//            fails. switch (sourceType->getKind()) {
//                case MetadataKind::ObjCClassWrapper:
//                    // Get the actual class object.
//                    sourceType = static_cast<const
//                    ObjCClassWrapperMetadata*>(sourceType)
//                        ->Class;
//                    LLVM_FALLTHROUGH;
//                case MetadataKind::Class: {
//                    // Check if the source is a subclass of the target.
//#if SWIFT_OBJC_INTEROP
//                    // We go through ObjC lookup to deal with potential
//                    runtime magic in ObjC
//                    // land.
//                    if (swift_dynamicCastObjCClassMetatype((const
//                    ClassMetadata*)sourceType, (const
//                    ClassMetadata*)targetType)) return origSourceType;
//#else
//                    if (_dynamicCastClassMetatype((const
//                    ClassMetadata*)sourceType,
//                                    (const ClassMetadata*)targetType))
//        return origSourceType;
//#endif
//                    return nullptr;
...
//
//        case MetadataKind::Existential: {
//            auto targetTypeAsExistential = static_cast<const
//            ExistentialTypeMetadata *>(targetType); if
//            (_conformsToProtocols(nullptr, sourceType,
//            targetTypeAsExistential, nullptr))
//                return origSourceType;
//            return nullptr;
//        }
//
...
//    }
//
//    return const_cast<void*>(swift_dynamicCastUnknownClass(object,
//    targetType));
//}
//
...
//    swift_dynamicCastUnknownClassUnconditional(object, targetType, nullptr, 0,
//    0));
//*destSlot = result;
//
// if (!(flags & DynamicCastFlags::TakeOnSuccess)) {
...
// return checkDynamicCastFromOptional(dest, src, payloadType, targetType,
// flags);
//}
//
///******************************************************************************/
...
//    //                                              inout result: Self?) ->
//    Bool SWIFT_CC(swift) bool (*conditionallyBridgeFromObjectiveC)(
//        HeapObject *sourceValue,
//        OpaqueValue *result,
//        SWIFT_CONTEXT const Metadata *self,
//        const Metadata *selfType,
...
//    // Class and ObjC existential metatypes can be bridged, but metatypes
//    can't
//    // directly conform to protocols yet. Use a stand-in conformance for a
//    type
...
// HeapObject *_swift_extractDynamicValue(OpaqueValue *value, const Metadata
// *self) {
//  OpaqueValue *outValue;
//  const Metadata *outType;
//  bool canTake = false;
