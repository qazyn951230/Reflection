...
_swift_getClassOfAllocated(const void *object) {
#if SWIFT_HAS_OPAQUE_ISAS
...
static inline bool objectUsesNativeSwiftReferenceCounting(const void *object) {
  ...
  return usesNativeSwiftReferenceCounting(
      _swift_getClassOfAllocatedFromPointer(object));
#else
...
static inline bool classHasSuperclass(const ClassMetadata *c) {
...
bool _contextDescriptorMatchesMangling(const ContextDescriptor *context,
...
void _forEachProtocolConformanceSectionAfter(
    size_t *start,
    const std::function<void(const ProtocolConformanceRecord *,
                             const ProtocolConformanceRecord *)> &f);

Demangle::NodePointer
_swift_buildDemanglingForMetadata(const Metadata *type,
...
TypeInfo swift_getTypeByMangledNode(
    MetadataRequest request, Demangler &demangler, Demangle::NodePointer node,
    const void *const *arguments, SubstGenericParameterFn substGenericParam,
...
SWIFT_CC(swift)
TypeInfo
swift_getTypeByMangledName(MetadataRequest request, StringRef typeName,
                           const void *const *arguments,
...
bool _gatherGenericParameterCounts(
    const ContextDescriptor *descriptor,
    llvm::SmallVectorImpl<unsigned> &genericParamCounts, Demangler &BorrowFrom);
...
template <class T> static void assignUnlessEqual(T &dest, T newValue) {
  if (dest != newValue)
    dest = newValue;
}
...
template <> inline const ClassMetadata *Metadata::getClassObject() const {
  switch (getKind()) {
...
public:
  explicit ResolveToDemanglingForContext(Demangle::Demangler &Dem) : Dem(Dem) {}
...
public:
  explicit ResolveAsSymbolicReference(Demangle::Demangler &Dem) : Dem(Dem) {}
...
bool _conformsToProtocol(const OpaqueValue *value, const Metadata *type,
                         ProtocolDescriptorRef protocol,
...
const Metadata *
findConformingSuperclass(const Metadata *type,
