...
//        // Resolve symbolic references to type contexts into the absolute
//        address of
//        // the type context descriptor, so that if we see a symbolic reference
//        in
...
class Resolution {
public:
  Resolution() : _demangler(new Demangler) {}

  Demangler &demangler() { return *_demangler; }
  [[nodiscard]] const Demangler &demangler() const { return *_demangler; }
  ...
  NodePointer demangleTypeRef(StringRef symbolName) {
    ...
    // the mangled name we can immediately find the associated metadata.
    return _demangler->demangleType(symbolName,
                                    ResolveAsSymbolicReference(*_demangler));
  }

private:
  std::unique_ptr<Demangler> _demangler;
};

template <size_t SIZE = 0>
class DemanglerForRuntimeTypeResolution : public Resolution {
public:
  DemanglerForRuntimeTypeResolution() : Resolution() {
    demangler().providePreallocatedMemory(StackSpace, SIZE);
  }
...
template <> class DemanglerForRuntimeTypeResolution<0> : public Resolution {};

NodePointer ResolveAsSymbolicReference::operator()(SymbolicReferenceKind kind,
...
static NodePointer _buildDemanglingForSymbolicReference(
    SymbolicReferenceKind kind, const void *resolvedReference, Demangler &Dem) {
  switch (kind) {
...
ResolveToDemanglingForContext::operator()(SymbolicReferenceKind kind,
                                          Directness isIndirect, int32_t offset,
                                          const void *base) {
  auto ptr = detail::applyRelativeOffset(base, offset);
  if (isIndirect == Directness::Indirect) {
    ptr = *(const uintptr_t *)ptr;
...
struct TypeMetadataSection {
  ...
  const TypeMetadataRecord *begin() const { return Begin; }
  const TypeMetadataRecord *end() const { return End; }
};
...
public:
  ...
  const ContextDescriptor *getDescription() { return Description; }
...
struct TypeMetadataPrivateState {
  ...
  TypeMetadataPrivateState() { initializeTypeMetadataRecordLookup(); }
};

static Lazy<TypeMetadataPrivateState> TypeMetadataRecords;

static void _registerTypeMetadataRecords(TypeMetadataPrivateState &T,
...
static void _forEachTypeMetadataSectionAfter(
    TypeMetadataPrivateState &T, size_t *start,
...
void swift::addImageTypeMetadataRecordBlockCallbackUnsafe(
  ...
  auto recordsBegin = reinterpret_cast<const TypeMetadataRecord *>(records);
  auto recordsEnd =
  ...
  // TypeMetadataRecords.
  _registerTypeMetadataRecords(
      TypeMetadataRecords.unsafeGetAlreadyInitialized(), recordsBegin,
      recordsEnd);
}
...
void swift::swift_registerTypeMetadataRecords(const TypeMetadataRecord *begin,
                                              const TypeMetadataRecord *end) {
...
_findContextDescriptor(Demangle::NodePointer node, Demangle::Demangler &Dem);
...
static const ContextDescriptor *_findExtendedTypeContextDescriptor(
    const ContextDescriptor *maybeExtension, Demangler &demangler,
    Demangle::NodePointer *demangledNode = nullptr) {
...
bool swift::_contextDescriptorMatchesMangling(const ContextDescriptor *context,
  ...
  while (context) {
    ...
    case ContextDescriptorKind::Extension: {
      ...
      DemanglerForRuntimeTypeResolution<> resolution;
      auto &demangler = resolution.demangler();
      ...
      auto extendedDescriptorFromDemangled = _findExtendedTypeContextDescriptor(
          extension, demangler, &extendedContextDemangled);

      // Determine whether the contexts match.
      bool contextsMatch = extendedDescriptorFromNode &&
...
// scanned, if any.
static void _scanAdditionalContextDescriptors(TypeMetadataPrivateState &T) {
  _forEachTypeMetadataSectionAfter(
      T, &T.TypeContextDescriptorLastSectionScanned,
      [&T](const TypeMetadataRecord *Begin, const TypeMetadataRecord *End) {
...
#define DESCRIPTOR_MANGLING_(CHAR, SUFFIX) $sS##CHAR##SUFFIX
#define DESCRIPTOR_MANGLING(CHAR, SUFFIX) DESCRIPTOR_MANGLING_(CHAR, SUFFIX)

#define STANDARD_TYPE(KIND, MANGLING, TYPENAME)                                \
  extern "C" const ContextDescriptor DESCRIPTOR_MANGLING(                      \
...
#include "swift/Demangling/StandardTypesMangling.def"
  ...
  if (foundContext) {
    ...
    //#ifndef NDEBUG
    //        // If we found something in the slow path but not in the cache, it
    //        is a
    //        // bug in the cache. Fail in assertions builds.
    //        if (!foundInCache) {
    //            fatalError(0,
    //                "_findContextDescriptor cache miss for demangled
...
struct ProtocolSection {
  ...
  const ProtocolRecord *begin() const { return Begin; }
  const ProtocolRecord *end() const { return End; }
};
...
struct ProtocolMetadataPrivateState {
  ...
  ProtocolMetadataPrivateState() { initializeProtocolLookup(); }
};

static Lazy<ProtocolMetadataPrivateState> Protocols;
} // namespace

static void _registerProtocols(ProtocolMetadataPrivateState &C,
...
void swift::addImageProtocolsBlockCallbackUnsafe(const void *protocols,
  ...
  auto recordsBegin = reinterpret_cast<const ProtocolRecord *>(protocols);
  auto recordsEnd =
      reinterpret_cast<const ProtocolRecord *>(protocolsBytes + protocolsSize);

  // Conformance cache should always be sufficiently initialized by this point.
  _registerProtocols(Protocols.unsafeGetAlreadyInitialized(), recordsBegin,
                     recordsEnd);
}
...
_searchProtocolRecords(ProtocolMetadataPrivateState &C, NodePointer node) {
...
static const ProtocolDescriptor *
_findProtocolDescriptor(NodePointer node, Demangle::Demangler &Dem,
...
bool swift::_gatherGenericParameterCounts(
    ...
    SmallVectorImpl<unsigned> &genericParamCounts, Demangler &BorrowFrom) {
  DemanglerForRuntimeTypeResolution<> resolution;
  auto &demangler = resolution.demangler();
  demangler.providePreallocatedMemory(BorrowFrom);

  if (auto extension =
          _findExtendedTypeContextDescriptor(descriptor, demangler)) {
...
static ArrayRef<GenericParamDescriptor>
...
static bool _gatherGenericParameters(
    const ContextDescriptor *context, ArrayRef<const Metadata *> genericArgs,
    const Metadata *parent, SmallVectorImpl<unsigned> &genericParamCounts,
    SmallVectorImpl<const void *> &allGenericArgsVec, Demangler &demangler) {
  // Figure out the various levels of generic parameters we have in
  // this type.
  (void)_gatherGenericParameterCounts(context, genericParamCounts, demangler);
  ...
  if (!genericParamCounts.empty()) {
    ...
    allGenericArgs.insert(allGenericArgs.end(), genericArgs.begin(),
                          genericArgs.end());
    ...
    SubstGenericParametersFromWrittenArgs substitutions(allGenericArgs,
                                                        genericParamCounts);
    bool failed = _checkGenericRequirements(
...
Optional<const ProtocolRequirement *>
...
_findOpaqueTypeDescriptor(NodePointer demangleNode, Demangler &dem) {
...
public:
  DecodedMetadataBuilder(Demangler &demangler,
                         SubstGenericParameterFn substGenericParameter,
                         SubstDependentWitnessTableFn substWitnessTable)
      : demangler(demangler), substGenericParameter(substGenericParameter),
  ...
  BuiltType resolveOpaqueType(NodePointer opaqueDecl,
    ...
    return swift_getTypeByMangledName(
               MetadataState::Complete, mangledName, allGenericArgsVec.data(),
               [&substitutions](unsigned depth, unsigned index) {
                 return substitutions.getMetadata(depth, index);
               },
               [&substitutions](const Metadata *type, unsigned index) {
  ...
  BuiltTypeDecl createTypeDecl(NodePointer node, bool &typeAlias) const {
  ...
  BuiltProtocolDecl createProtocolDecl(NodePointer node) const {
    ...
    //#if SWIFT_OBJC_INTEROP
    //        // Look for a Swift-defined @objc protocol with the Swift 3
    //        mangling that
    //        // is used for Objective-C entities.
    //        const char *objcMangledName = mangleNodeAsObjcCString(node,
    //        demangler); if (auto protocol = objc_getProtocol(objcMangledName))
  ...
  BuiltProtocolDecl
  createObjCProtocolDecl(const std::string &mangledName) const {
#if SWIFT_OBJC_INTEROP
  ...
  BuiltType createBoundGenericType(BuiltTypeDecl anyTypeDecl,
    ...
    if (!_gatherGenericParameters(typeDecl, genericArgs, parent,
...
#include "swift/Runtime/BuiltinTypes.def"
  ...
  BuiltType
  createFunctionType(ArrayRef<Demangle::FunctionParam<BuiltType>> params,
  ...
  BuiltType createTupleType(ArrayRef<BuiltType> elements, std::string labels,
                            bool variadic) const {
    ...
    return swift_getTupleTypeMetadata(MetadataState::Abstract, flags,
                                      elements.data(),
  ...
  BuiltType createDependentMemberType(StringRef name, BuiltType base,
    ...
    return swift_getAssociatedTypeWitness(
               ...
               const_cast<WitnessTable *>(witnessTable), base,
               swiftProtocol->getRequirementBaseDescriptor(), *assocType)
        .Value;
  }
...
swift_getTypeByMangledNameImpl(MetadataRequest request, StringRef typeName,
                               ...
                               SubstGenericParameterFn substGenericParam,
                               SubstDependentWitnessTableFn substWitnessTable) {
  //    DemanglerForRuntimeTypeResolution<StackAllocatedDemangler<2048>>
  //    demangler;
  DemanglerForRuntimeTypeResolution<2048> resolution;
  auto &demangler = resolution.demangler();
  ...
  if (dotPos != llvm::StringRef::npos) {
    ...
    NodePointer moduleNode =
        demangler.createNode(Node::Kind::Module, typeName.substr(0, dotPos));
    ...
    node = resolution.demangleTypeRef(typeName);
    if (!node)
      return TypeInfo();
  }

  return swift_getTypeByMangledNode(request, demangler, node,
                                    origArgumentVector, substGenericParam,
                                    substWitnessTable);
}
...
//  return swift_getTypeByMangledName(
//             MetadataState::Complete, typeName, genericArgs,
//             [&substitutions](unsigned depth, unsigned index) {
//               return substitutions.getMetadata(depth, index);
//             },
//             [&substitutions](const Metadata *type, unsigned index) {
...
//  SubstGenericParametersFromMetadata substitutions(environment, genericArgs);
//  return swift_getTypeByMangledName(
//             (MetadataState)metadataState, typeName, genericArgs,
//             [&substitutions](unsigned depth, unsigned index) {
//               return substitutions.getMetadata(depth, index);
//             },
//             [&substitutions](const Metadata *type, unsigned index) {
...
//
//  return swift_getTypeByMangledName(
//             request.getState(), mangledName, arguments,
//             [&substitutions](unsigned depth, unsigned index) {
//               return substitutions.getMetadata(depth, index);
//             },
//             [&substitutions](const Metadata *type, unsigned index) {
...
//      return NO;
//    metadata =
//        swift_getTypeByMangledNode(
//            MetadataState::Complete, demangler, node, nullptr,
//            /* no substitutions */
...
//  }
//  if (metadata) {
//    auto objcClass = reinterpret_cast<Class>(const_cast<ClassMetadata *>(
...
//__attribute__((constructor)) static void installGetClassHook() {
//  // FIXME: delete this #if and dlsym once we don't
//  // need to build with older libobjc headers
//#if !OBJC_GETCLASSHOOK_DEFINED
//  using objc_hook_getClass =
//      BOOL (*)(const char *_Nonnull name, Class _Nullable *_Nonnull outClass);
//  auto objc_setHook_getClass = (void (*)(
//      objc_hook_getClass _Nonnull, objc_hook_getClass _Nullable * _Nonnull))
...
unsigned SubstGenericParametersFromMetadata::buildDescriptorPath(
    const ContextDescriptor *context, Demangler &borrowFrom) const {
  ...
  auto localGenericParams =
      allGenericParams.slice(parentCount, localCount - parentCount);
  ...
  if (localCount > parentCount)
    descriptorPath.push_back(
        PathElement{localGenericParams, context->getNumGenericParams(),
                    numKeyGenericParamsInParent, numKeyGenericParamsHere,
...
unsigned SubstGenericParametersFromMetadata::buildEnvironmentPath(
  ...
  for (unsigned numLocalParams : environment->getGenericParameterCounts()) {
    ...
    descriptorPath.push_back(PathElement{
        localGenericParams, totalParamCount, numKeyGenericParamsInParent,
        numKeyGenericParamsHere, hasNonKeyGenericParams});
  }
...
const Metadata *
SubstGenericParametersFromMetadata::getMetadata(unsigned depth,
                                                unsigned index) const {
...
const Metadata *
SubstGenericParametersFromWrittenArgs::getMetadata(unsigned depth,
                                                   unsigned index) const {
  if (auto flatIndex =
...
void swift::gatherWrittenGenericArgs(
    ...
    SmallVectorImpl<const Metadata *> &allGenericArgs, Demangler &BorrowFrom) {
  if (!description)
    return;
  ...
  for (const auto &req : generics->getGenericRequirements()) {
    ...
    auto lhsFlatIndex = _depthIndexToFlatIndex(
        lhsParam->first, lhsParam->second, genericParamCounts);
    ...
    auto rhsFlatIndex = _depthIndexToFlatIndex(
        rhsParam->first, rhsParam->second, genericParamCounts);
...
struct InitializeDynamicReplacementLookup {
  InitializeDynamicReplacementLookup() { initializeDynamicReplacementLookup(); }
};
