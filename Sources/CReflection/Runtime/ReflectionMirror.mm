...
struct TupleImpl : ReflectionMirrorImpl {
  ...
  AnyReturn subscript(intptr_t i, const char **outName,
    ...
    *outFreeFunc = [](const char *str) { free(const_cast<char *>(str)); };
    ...
    auto *opaqueValueAddr =
        result.Type->allocateBoxForExistentialIn(&result.Buffer);
...
static void missing_reflection_metadata_warning(const char *fmt, ...) {
  bool shouldWarn =
...
getFieldAt(const Metadata *base, unsigned index) {
  ...
  auto typeInfo = swift_getTypeByMangledName(
  ...
  if (!typeInfo.getMetadata()) {
    typeInfo = TypeInfo(
        {&METADATA_SYM(EMPTY_TUPLE_MANGLING), MetadataState::Complete}, {});
    missing_reflection_metadata_warning(
...
struct EnumImpl : ReflectionMirrorImpl {
  ...
  AnyReturn subscript(intptr_t i, const char **outName,
    ...
    boxType->vw_initializeWithCopy(pair.buffer,
...
struct ClassImpl : ReflectionMirrorImpl {
  ...
  AnyReturn subscript(intptr_t i, const char **outName,
    ...
    // FIXME: If the class has ObjC heritage, get the field offset using the
    // ObjC metadata, because we don't update the field offsets in the face of
...
struct MetatypeImpl : ReflectionMirrorImpl {
  char displayStyle() { return '\0'; }

  intptr_t count() { return 0; }
...
template <typename F>
auto call(OpaqueValue *passedValue, const Metadata *T,
          const Metadata *passedType, const F &f) -> decltype(f(nullptr)) {
  const Metadata *type;
  OpaqueValue *value;
...
SWIFT_CC(swift)
SWIFT_RUNTIME_STDLIB_API const Metadata *swift_reflectionMirror_normalizedType(
    OpaqueValue *value, const Metadata *type, const Metadata *T) {
  return call(value, T, type,
              [](ReflectionMirrorImpl *impl) { return impl->type; });
}

// func _getChildCount<T>(_: T, type: Any.Type) -> Int
SWIFT_CC(swift)
SWIFT_RUNTIME_STDLIB_API intptr_t
    swift_reflectionMirror_count(OpaqueValue *value, const Metadata *type,
                                 const Metadata *T) {
  return call(value, T, type,
              [](ReflectionMirrorImpl *impl) { return impl->count(); });
}
...
// ) -> Any
SWIFT_CC(swift)
SWIFT_RUNTIME_STDLIB_API AnyReturn
...
SWIFT_RUNTIME_STDLIB_API
    char swift_reflectionMirror_displayStyle(OpaqueValue *value,
                                             const Metadata *T) {
  return call(value, T, nullptr,
              [](ReflectionMirrorImpl *impl) { return impl->displayStyle(); });
}

// func _getEnumCaseName<T>(_ value: T) -> UnsafePointer<CChar>?
SWIFT_CC(swift)
SWIFT_RUNTIME_STDLIB_API const
    char *swift_EnumCaseName(OpaqueValue *value, const Metadata *T) {
  return call(value, T, nullptr,
              [](ReflectionMirrorImpl *impl) { return impl->enumCaseName(); });
}

// func _opaqueSummary(_ metadata: Any.Type) -> UnsafePointer<CChar>?
SWIFT_CC(swift)
SWIFT_RUNTIME_STDLIB_API const char *swift_OpaqueSummary(const Metadata *T) {
  switch (T->getKind()) {
  case MetadataKind::Class:
  case MetadataKind::Struct:
  case MetadataKind::Enum:
...
SWIFT_CC(swift)
SWIFT_RUNTIME_STDLIB_API id
    swift_reflectionMirror_quickLookObject(OpaqueValue *value,
                                           const Metadata *T) {
  return call(value, T, nullptr, [](ReflectionMirrorImpl *impl) {
    return impl->quickLookObject();
  });
}
#endif
