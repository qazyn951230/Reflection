...
template <class T, size_t Alignment = alignof(T), size_t Size = sizeof(T),
...
struct NativeBox {
  ...
  static void destroy(T *value) { value->T::~T(); }

  static T *initializeWithCopy(T *dest, T *src) { return new (dest) T(*src); }
...
template <class Impl, class T> struct RetainableBoxBase {
  ...
  static void destroy(T *addr) { Impl::release(*addr); }
...
template <typename Impl, typename T> struct WeakRetainableBoxBase {
  using type = T;
...
struct SwiftWeakRetainableBox
  ...
  static void destroy(WeakReference *ref) { swift_weakDestroy(ref); }
...
struct ObjCRetainableBox : RetainableBoxBase<ObjCRetainableBox, void *> {
  ...
  static void *retain(void *obj) { return objc_retain((id)obj); }

  static void release(void *obj) { objc_release((id)obj); }
};
...
struct BridgeObjectBox : RetainableBoxBase<BridgeObjectBox, void *> {
  static void *retain(void *obj) { return swift_bridgeObjectRetain(obj); }

  static void release(void *obj) { swift_bridgeObjectRelease(obj); }
};
...
template <size_t StartOffset, class... EltBoxes> struct AggregateBoxHelper;

// Base case: empty list.
template <size_t StartOffset> struct AggregateBoxHelper<StartOffset> {
public:
...
public:
#define COPY_OP(OP)                                                            \
  static char *OP(char *dest, char *src) { return dest; }
...
public:
  ...
  static constexpr size_t alignment =
      (NextHelper::alignment > EltBox::alignment ? NextHelper::alignment
                                                 : EltBox::alignment);
...
template <class... EltBoxes> struct AggregateBox {
  using type = char;
  ...
  static void destroy(char *value) { Helper::destroy(value); }
...
/// and alignment.
template <size_t Size, size_t Alignment> struct SwiftAllocator {
  static void *alloc() { return swift_slowAlloc(Size, Alignment - 1); }
...
enum class FixedPacking { Allocate, OffsetZero };
...
struct NonFixedValueWitnesses
    : NonFixedBufferValueWitnesses<
          NonFixedValueWitnesses<Box, IsKnownAllocated>, IsKnownAllocated> {
  ...
  static OpaqueValue *initializeWithCopy(OpaqueValue *dest, OpaqueValue *src,
                                         const Metadata *self) {
    return (OpaqueValue *)Box::initializeWithCopy(
        (typename Box::type *)dest, (typename Box::type *)src, self);
  }

  static OpaqueValue *initializeWithTake(OpaqueValue *dest, OpaqueValue *src,
                                         const Metadata *self) {
    return (OpaqueValue *)Box::initializeWithTake(
        (typename Box::type *)dest, (typename Box::type *)src, self);
  }
...
template <class Witnesses> struct ValueWitnessTableGenerator {
